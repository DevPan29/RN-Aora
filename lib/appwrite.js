...
export const getAllPosts = async (user) => {
    try {
        ...
        // add isLiked state to each post
        const postsWithLikes = await Promise.all(
            posts.documents.map(async (post) => {
                const isLiked = await isVideoLikedByUser(user.$id, post.$id);
...
export const getUserPosts = async (userId) => {
    try {
        ...
        const postsWithLikes = await Promise.all(
            posts.documents.map(async (post) => {
                const isLiked = await isVideoLikedByUser(userId, post.$id);
                console.log(`isLiked`, isLiked);
                return {
                    ...post,
                    isLiked
...
export const isVideoLikedByUser = async (userId, videoId) => {
    try {
        ...
        console.error(`isVideoLikedByUser: ${error}`);
        return false;
    }
};
